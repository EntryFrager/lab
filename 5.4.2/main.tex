...
\]
И это приводит к спектру, имеющему вид широкого колокола (рис.~\ref{fig:spectrum}). Кривая плавно отходит от нуля и столь же плавно, по параболе, касается оси абсцисс в области максимального импульса электронов.
...
Далее построим график зависимости числа отсчетов от импульса электронов. Для этого пересчитаем ток в импульс по такой формуле:
...
\[
pc = \sqrt{(T_k + mc^2)^2 - (mc^2)^2} = \sqrt{T_k^2 + 2T_{k}mc^2} \approx 1013.5 \text{ кэВ/c - импульс конверсионных электронов}
\]

Далее определим значение силы тока при котором у нас наблюдается конверсионный пик и отобразим это на графике.
...
Таким образом, $I_k = 4.26 \pm 0.01$ А. Определим коэффициент k:
...
Таким образом, мы получаем, что $p_{peak} = 1013 \pm 7 \; \frac{\text{кэВ}}{c}$. Далее переведем значения импульсов в энергию и построим график зависимости $N(T_e)$.
